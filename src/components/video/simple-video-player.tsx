...
export function SimpleVideoPlayer({
  ...
  const defaultAnalyticsProps = useMemo(
    () => ({ courseId, lessonId, videoId }),
    [courseId, lessonId, videoId],
  );
  const { capture } = usePosthogAnalytics({
    defaultProperties: defaultAnalyticsProps,
  });
  ...
  const youtubeId = useMemo(
    () => getYouTubeId(youtubeUrl || undefined),
    [youtubeUrl],
  );
  ...
  useEffect(() => {
    ...
    let chosenSource: string | null =
      streamUrl || (isLikelyPlayableUrl(url) ? url! : null);
    if (!chosenSource && !streamUrl && url && !isLikelyPlayableUrl(url)) {
      chosenSource = FALLBACK_VIDEO_URL;
      setStreamUrl(FALLBACK_VIDEO_URL);
      capture("lesson_video_fallback_applied", {
        properties: {
          reason: "unplayable_source",
          originalUrl: url,
        },
      });
    }
  ...
  }, [streamUrl, url, youtubeUrl, onProgress, onEnded, capture]);
  ...
  if (videoId && !currentVideoUrl && !state.error && state.isLoading) {
    return (
      <div
        className={cn(
          "relative bg-gray-900 rounded-lg overflow-hidden flex items-center justify-center min-h-[400px]",
          className,
        )}
      >

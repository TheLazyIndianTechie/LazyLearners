...
export async function processPayment(
  ...
  try {
    ...
    return { success: false, error: "An error occurred processing your payment" }
  }
}
...
async function updateCartTotals(cartId: string): Promise<Cart | null> {
  try {
    ...
    const subtotal = cart.items.reduce((sum, item) => sum + (item.discountedPrice || item.price), 0)
...
export async function applyCoupon(
  ...
  try {
    ...
    if (coupon.minimumAmount && cart.subtotal < coupon.minimumAmount) {
      return { success: false, error: `Minimum order amount of $${coupon.minimumAmount} required` }
    }
  ...
  try {
    ...
    const subscription = await prisma.subscription.create({
      data: {
        ...
        currentPeriodEnd: getNextBillingDate(new Date(), plan.interval, plan.intervalCount),
...
function getNextBillingDate(startDate: Date, interval: string, intervalCount: number): Date {
